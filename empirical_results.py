...
def analyze_day(lambdas, dataset, indexer, symbol, 
    ...
    dataset.loc[indexer, 'StaticSol'] =\
        StaticSolution(C, 0., static_model_params['M_t'], s_t, np.ones(T), sigma_t)
    
    dynamic_sols = dict([(el, np.zeros(T)) for el in lambdas]) 
    solver = SHDPSolution(s_t, C, alpha, sigma_t)#, predictor)
    logger.debug("Running dynamic solution for %d values of lambda."%len(lambdas))
    for t in range(T):
    ...
    for lambda_var in lambdas:
        dataset.loc[indexer, "DynamicSol%.2e"%lambda_var] = dynamic_sols[lambda_var]
...
def aggregate_results(dataset, alpha, spread):
    ...
    solutions = ['StaticSol'] + [el for el in dataset.columns if el[:7] == 'Dynamic'] 
        ...
        for symbol in ALL_SYMBOLS:
            ...
            for i,solution in enumerate(solutions):
                Costs[i] = -sum(spread*sample_day[solution])/(2.*C) + \
                        sum(spread*(sample_day[solution]**2/sample_day.Volume))*alpha/(2.*C)
                solution_VWAP = sum(sample_day[solution]*sample_day.Price)/C
                Stildes[i] = ((solution_VWAP - market_VWAP)/market_VWAP) + Costs[i]
            result_DF.loc[len(result_DF)] = [symbol, day] + list(Stildes) + list(Costs)

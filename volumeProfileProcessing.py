...
kContinousCondition = set(['', '@', 'F', '@F', 'C', 'CF', 'N', 'NF', 'R', 'RF', 'Q', 'M'])
kOpenCondition = set(['O', '@O'])
kCloseCondition = set(['6', '@6'])
kInValidCondition = set(['Z', '4', '@4', 'C4', 'N4', 'R4', 'T', 'V', 'U', 'P', 'W', 'X'])
...
class MarketTradingDay(object):
	...
	def __init__(self, symbol, day, interval_lenght = 1):
		...
		self.prices = np.zeros(390/interval_lenght + 2)*np.nan #set them to NaN
		self.times_list = self.compute_times()
		self.activeTradingIntervalIndex = 1
		self.activeTradingIntervalTime = self.times_list[self.activeTradingIntervalIndex]
		self.activeTradingInterval = ContinuousTradingInterval(self.activeTradingIntervalTime)
	...
	def compute_times(self):
		...
		times_list = [(base + datetime.timedelta(minutes=self.interval_lenght*x)).time() for x in range(0, numint)]
		times_list = [datetime.time(9,29,00)] + times_list + [datetime.time(16,00,00)]
		return times_list
...
def readFile(fileObj, nrows = None):
	...
	for index, line in enumerate(fileObj):
		...
		if day in kExcludeDays: continue
